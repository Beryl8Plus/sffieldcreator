...
def build_metadata_for_field(field_data):
	...
	if field_type == 'Checkbox':
		...
		metadata['precision'] = int(field_data['precision']) + 2 if field_data['precision'] else None
		...
		metadata['length'] = field_data['length']
		metadata['required'] = field_data['required']
		metadata['maskChar'] = field_date['maskchar']
		metadata['maskType'] = field_date['masktype']
...
def build_picklist_values(field_data):
	...
	try:
		...
		for i, picklist in picklist_values_list:

			# Build picklist value
			json_dict = {
				'valueName': picklist,
				'default': True if i == 0 and field_data['firstvaluedefault'] else False # If first value and first value default is checked
			} 
